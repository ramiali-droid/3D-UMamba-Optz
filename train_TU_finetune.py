"""
Adapted for S3DIS 8-class training/augmenting/fine-tuning
High-performance version for powerful GPU
"""
import argparse
import os
from data_utils.TUBBlockDataLoader_v3 import TUBDataset  # ← updated
import torch
import datetime
import logging
...
class ConfigS3DIS:
    ...
    augment_symmetries        = [True, False, False]  # x, y, z symmetry
    augment_rotation          = 'vertical'            # or 'none', 'all'
    augment_scale_min         = 0.8
    augment_scale_max         = 1.2
    augment_noise             = 0.001                 # Gaussian noise std
...
def load_state_dict_ignore_channels(model, ckpt_path, strict=False):
    ...
    for k, v in state_dict.items():
        ...
        if v.shape == model_dict[k].shape:
            ...
            # Handle channel mismatch (assumes channel dim = 1)
...
def main(args):
    ...
    root = '/home/ramiali/3dumamba/data/TUB/combined/crt'
    ...
    try:
        ...
        load_state_dict_ignore_channels(classifier, str(experiment_dir) + '/checkpoints/pre_model.pth')
    ...
    layers_to_train = ['mamba3', 'sa4', 'fp4', 'fp3', 'fp2', 'fp1', 'conv1', 'bn1', 'drop1', 'conv2']
    for name, param in classifier.named_parameters():
    # Check if the parameter belongs to a layer we want to freeze
        if any(layer_name in name for layer_name in layers_to_train):
            param.requires_grad = True
        else:
            param.requires_grad = False
    
    # for name, layer in classifier.named_parameters():
    #    print(f"layer name: {name}: {layer}, {layer.requires_grad}")
    ...
    for fold in range(1):
        print("start loading training data ...")
        TRAIN_DATASET = TUBDataset(split='train', data_root=root, npoints=NUM_POINT)
        
        print("start loading test data ...")
        TEST_DATASET = TUBDataset(split='test', data_root=root, npoints=NUM_POINT)
