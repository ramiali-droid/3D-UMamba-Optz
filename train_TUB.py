...
import argparse
import os
from data_utils.TUBBlockDataLoader_v3_SIMPLE import TUBDataset  # ← Simple single split (no CV)
import torch
import datetime
import logging
...
def parse_args():
    ...
    default_log_dir = f'tub_{berlin_iso}'
...
def main(args):
    ...
    experiment_dir = experiment_dir.joinpath('tub_8class_rad_0.5')
    experiment_dir.mkdir(exist_ok=True)
    ...
    try:
        ...
        checkpoint = torch.load('/home/ramiali/3dumamba/log/s3dis_8class_cv/sedis_2026-01-29/checkpoints/pre_m1odel.pth')
        start_epoch = 0
    ...
    # Remove fold loop - use single train/test split
    print("Loading training data (D1, D3, D5)...")
    TRAIN_DATASET = TUBDataset(split='train', data_root=root, npoints=NUM_POINT, augment=True)
    print("Loading test data (D2, D4)...")
    TEST_DATASET = TUBDataset(split='test', data_root=root, npoints=NUM_POINT, augment=False)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            ...
            writer.add_scalar(f"Loss/Eval", loss_sum / float(num_batches), epoch)
