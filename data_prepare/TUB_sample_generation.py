...
dataset_path = '/home/ramiali/3dumamba/data/TUB/combined/crt/D5_rooms_crt/filtered/subsampled_0.010/'
file_paths = glob.glob(dataset_path + '*.txt')  # collecting all files

def norm_and_surface_variance(pc, neighbor_radius=0.5):  #changed from 1.0 to 0.5
    """
    ...
    point_cloud = pc[:, 6:9]       # ax, ay, az used for neighborhood PCA
    ...
    # output = [points (pc[:,:9], shape 9), norm_features (3), surface_variance (1), labels (1)] -> total 14
    out = np.concatenate([points[:, :9], norm_features, surface_variance, labels], axis=-1)
    return out
...
for file_path in tqdm(file_paths):
    ...
    for size_ in range(len(block_size_list)):
        ...
        min_npoints = 8192 #8192

        data_folder = dataset_path + 'Block_s' + str(block_size) + '_min_final_' + str(min_npoints) + '_norm_enhance_rad_0.5/'
        ...
        print("blocks:", len(center_x)*len(center_y))
        ...
        for i in range(len(coordinates)):
            ...
            """
        data_npy = []
        print(f"Room bounds: x {coord_min[0]:.2f} → {coord_max[0]:.2f}, y {coord_min[1]:.2f} → {coord_max[1]:.2f}")

        for i in range(len(coordinates)):
            cx, cy = coordinates[i]
            block_x_min = cx - block_size / 2
            block_x_max = cx + block_size / 2
            block_y_min = cy - block_size / 2
            block_y_max = cy + block_size / 2

            print(f"Block {i:2d}: center ({cx:.2f}, {cy:.2f}), bounds x {block_x_min:.2f}→{block_x_max:.2f}, y {block_y_min:.2f}→{block_y_max:.2f}")

            point_idxs = np.where(
                (points[:, 0] >= block_x_min) & (points[:, 0] <= block_x_max) &
                (points[:, 1] >= block_y_min) & (points[:, 1] <= block_y_max)
            )[0]

            print(f"  Points inside: {point_idxs.size}")
            ...
            current_points[:, :3] = selected_points[:, :3]   # x,y,z
            current_points[:, 3:6] = selected_points[:, 3:6]  # r,g,b
            current_points[:, 6:9] = selected_points[:, :3]   # extra coords (ax,ay,az)
            current_points[:, 9] = 0                         # placeholder ny                    # placeholder nz
            current_points[:, 10] = selected_points[:, 6]   # label


            current_points = norm_and_surface_variance(current_points)

            current_points = np.expand_dims(current_points, axis=0)
            current_points = coordinate_normalize(current_points)  # rx ry rz r g b ax ay az nx ny nz sv label
