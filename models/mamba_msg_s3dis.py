...
class get_model(nn.Module):
    ...
    def forward(self, xyz, fps_index_array, series_idx_arrays): #（B C N）， （B 3 N）， （B, 3, 8 N）
        ...
        # xyz1 = xyz[:, :4, :] # - Dales
        # rx, ry, rz, intensity, ax, ay, az, nx, ny, nz, sv - Dales
        
        # xyz2 = xyz[:, 7:, :] # 8 channels in total, delete the 3 absolute coordinate channels. raw input channel is 11 - Dales

        xyz1 = xyz[:, :6, :] # -S3DIS
        # rx, ry, rz, r, g, b, ax, ay, az, nx, ny, nz, sv - # -S3DIS
        xyz2 = xyz[:, 9:, :] # 10 channels in total, delete the 3 absolute coordinate channels. raw input channel is 13 - # -S3DIS
        xyz = torch.cat([xyz1,xyz2], dim=1) # -S3DIS
